...
% The Baseload_Umass_homeB_30min.m is used for load decomposition of residential customer in Western Massachusetts,USA
...
air6_es=min(air6_es,2.8);
air7_es=min(air7_es,2.8);
air8_es=min(air8_es,2.8);
air9_es=min(air9_es,2.8);
...
plot([air8(1,:) air8(2,:) air8(3,:) air8(4,:) air8(5,:) air8(6,:) air8(7,:)],'k-','LineWidth',1)
hold on
plot([air8_es(1,:) air8_es(2,:) air8_es(3,:) air8_es(4,:) air8_es(5,:) air8_es(6,:) air8_es(7,:)],'r-','LineWidth',1)
hold on
set(gca,'FontName','Times New Roman','FontSize',8)
...
air8_es_new=reshape(air8_es(1:7,:),size(air8_es(1:7,:),1)*size(air8_es(1:7,:),2),1);

air8_new=reshape(air8(1:7,:),size(air8(1:7,:),1)*size(air8(1:7,:),2),1);

figure (28)
set(gcf,'unit','centimeters','position',[0,0,8,6])

plot(air8_new,'k-','LineWidth',1)
hold on
plot(air8_es_new,'r-.','LineWidth',1)
hold on
set(gca,'FontName','Times New Roman','FontSize',8)
ylabel('\fontsize{8}\fontname{Times new roman}ACLs (kW)')
...
xlim([0 350])
hold on
axes('position',[0.25,0.7,0.4,0.2]);
hold on
plot(air8_new(140:240,1),'k-','LineWidth',1);
hold on
plot(air8_es_new(140:240,1),'r-.','LineWidth',1);
set(gca,'FontName','Times New Roman','FontSize',8)
ylabel('\fontsize{8}\fontname{Times new roman}ACLs (kW)')
xlim([0 100])
